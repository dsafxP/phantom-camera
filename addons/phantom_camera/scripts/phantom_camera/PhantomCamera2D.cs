...
public class PhantomCamera2D : PhantomCamera
{
    ...
    {
        get => Node2D.Call(PhantomCamera.MethodName.GetFollowTargets)
            .AsGodotArray<Node2D>()
            .ToArray();

        set => Node2D.Call(PhantomCamera.MethodName.SetFollowTargets, new Godot.Collections.Array<Node2D>(value));
    }
    ...
    public PhantomCameraNoise2D Noise
    {
        ...
        set => Node2D.Call(MethodName.SetNoise, value.Resource);
    }
    ...
    public static PhantomCamera2D FromScript(string path) => new(GD.Load<GDScript>(path)
        .New()
        .AsGodotObject());

    public static PhantomCamera2D FromScript(GDScript script) => new(script
        .New()
        .AsGodotObject());
    ...
    [System.Obsolete]
    public void SetLimitTarget(TileMap tileMap) => Node.Call(MethodName.SetLimitTarget, tileMap.GetPath());

    public void SetLimitTarget(TileMapLayer tileMapLayer) => Node.Call(MethodName.SetLimitTarget, tileMapLayer.GetPath());

    public void SetLimitTarget(CollisionShape2D shape2D) => Node.Call(MethodName.SetLimitTarget, shape2D.GetPath());

    public void SetNoiseEmitterLayerValue(int layer, bool enabled) => Node2D.Call(MethodName.SetNoiseEmitterLayerValue, layer, enabled);
    ...
    public void SetLimit(Side side, int value) => Node.Call(MethodName.SetLimit, (int)side, value);

    public int GetLimit(Side side) => (int)Node.Call(MethodName.GetLimit, (int)side);

    public void EmitNoise(Transform2D transform2D) => Node2D.Call(MethodName.EmitNoise, transform2D);
